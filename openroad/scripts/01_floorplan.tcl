...
#set bank1X [expr {$floor_leftX + ($RamSize2048x64_W/2)}]
#set bank1Y  [expr {$floor_bottomY + $RamSize2048x64_H}]
...
# Bank2: bottom-right, pins facing up
set bank2X [expr {$floor_rightX - ($RamSize512x32_W)}]
set bank2Y $floor_bottomY
placeInstance $bank2_sram0 $bank2X $bank2Y MX

# Bank3: bottom-left, pins facing up
set bank3X $floor_leftX
set bank3Y $floor_bottomY
placeInstance $bank3_sram1 $bank3X $bank3Y MX


utl::report "SRAM macro box: width ${RamSize512x32_W} height ${RamSize512x32_H}"
utl::report "Big SRAM macro box: width ${RamSize2048x64_W} height ${RamSize2048x64_H}"
...
utl::report "SRAM bank2 bbox: ($bank2X, $bank2Y) - ([expr {$bank2X + $RamSize512x32_W}], [expr {$bank2Y + $RamSize512x32_H}]) MX"
utl::report "SRAM bank3 bbox: ($bank3X, $bank3Y) - ([expr {$bank3X + $RamSize2048x64_W}], [expr {$bank3Y + $RamSize2048x64_H}]) MX"      
